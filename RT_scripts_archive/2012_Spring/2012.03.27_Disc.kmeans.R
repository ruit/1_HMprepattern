...
##############################
###March 27, 2012. Bugs unsolved !!!!!!??????Get_level2<-function(dnt.lev, br.vec){
#dnt.vec is the HM densities
#br.vec is the breakpoints determined by k-means
	
#	k<-length(br.vec)/2
#	for (i in 1:k){
#		print (length(dnt.lev[dnt.lev>=br.vec[(2*i-1)] & dnt.lev<=br.vec[(2*i)]]))
		
#		dnt.lev[dnt.lev>=br.vec[(2*i-1)] & dnt.lev<=br.vec[(2*i)]]<-paste("Lev", i, sep="")###Must be character!!!!!!
