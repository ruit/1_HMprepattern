...
#pseudo<-c(0.0001)#10e-4
	...
	### Jan. 5, 2012
	### for tag density take log and standardize (center) the data
	
	dataTcell.log.centered<-scale(log2(dataTcell+pseudo))
	rm (dataTcell)
	
	classesTcell<-c(rep("1",posi_sample_n), rep("0",posi_sample_n))
	data<-data.frame(dataTcell.log.centered, class=classesTcell)
	return (data)
	rm (induced)
	rm (SS)
	}
